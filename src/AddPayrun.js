...
const AddPayrun = () => {
  ...
  const handlePrintSelectedRows = async () => {
    ...
    const selectedRows = selectionModel.map((rowId) =>
    ...
    const recordsArray = Object.entries(dateToSumMap).map(
      ([date, { total_hours, total_km, total_remittance }]) => ({
    ...
    console.log(
      "Sum of hours, travel time, and travel km by date:",
      dateToSumMap
    );
    ...
    if (userConfirmed) {
      try {
        ...
        const response = await fetch(`${BACKEND_URL}/store_payrun_data`, {
          method: "POST",
          headers: {
  ...
  const handleSearchButtonClick = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/invoices/employee?start_date=${formattedStartDate}&end_date=${formattedEndDate}&employee=${employeeName.trim()}`
      );
